...
import (
	...
	_ "github.com/PhenixChain/devchain/system/consensus/init"
...
func initEnv3() (queue.Queue, queue.Module, queue.Module, *Mempool) {
	...
	cfg, sub := types.InitCfg("../../cmd/autotest/chain33.test.toml")
...
func initEnv(size int) (queue.Queue, *Mempool) {
	...
	var q = queue.New("channel")
	cfg, _ := types.InitCfg("../../cmd/autotest/chain33.test.toml")
	types.Init(cfg.Title, cfg)
	blockchainProcess(q)
	execProcess(q)

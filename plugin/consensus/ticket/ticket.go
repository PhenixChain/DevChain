...
import (
	...
	"github.com/PhenixChain/devchain/common/difficulty"
	"github.com/PhenixChain/devchain/common/log/log15"
	vrf "github.com/PhenixChain/devchain/common/vrf/secp256k1"
	"github.com/PhenixChain/devchain/queue"
	drivers "github.com/PhenixChain/devchain/system/consensus"
	driver "github.com/PhenixChain/devchain/system/dapp"
	cty "github.com/PhenixChain/devchain/system/dapp/coins/types"
...
func (client *Client) CreateGenesisTx() (ret []*types.Transaction) {
	// *** 系统初始地址 nikolas ***
	txSys := types.Transaction{}
	txSys.Execer = []byte("coins")
	txSys.To = client.Cfg.Genesis
	gSys := &cty.CoinsAction_Genesis{}
	//自定义总额
	gSys.Genesis = &types.AssetsGenesis{Amount: 3000000000 * types.Coin}
	txSys.Payload = types.Encode(&cty.CoinsAction{Value: gSys, Ty: cty.CoinsActionGenesis})
	ret = append(ret, &txSys)
...
func (client *Client) searchTargetTicket(parent, block *types.Block) (*ty.Ticket, crypto.PrivKey, *big.Int, []byte, string, error) {
	...
	for ticketID, ticket := range client.ticketsMap {
		...
		tlog.Info("FindBlock", "⛏️ block height------->", block.Height, "ntx", len(block.Txs))
